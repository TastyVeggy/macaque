...
module activate_unit (
  ...
  // Datapath: 3-stage register pipeline:
  //   Stage C (act+clamp)  : qb_wdata = clamp(func(requant_r)); drives qb_we/qb_waddr
  logic signed [63:0] prod_r    [npu_pkg::ARRAY_SIZE];  // Stage A out
  logic signed [63:0] requant_r [npu_pkg::ARRAY_SIZE];  // Stage B out
  ...
  assign s1_consume = (state == ACT_PIPELINE) &&
                      (act_count > 0 && act_count <= num_rows);
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      // Stage 2 (round+shift): requant_r = (prod_r + round) >>> shift
      if (valid_a) begin
        for (int i = 0; i < npu_pkg::ARRAY_SIZE; i++) begin
          automatic logic signed [63:0] scaled;
          scaled = prod_r[i];
          if (scale_shift > 0)
            scaled = prod_r[i] + (64'sd1 << (scale_shift - 1));
          requant_r[i] <= scaled >>> scale_shift;
        end
      end
      valid_b <= valid_a;
      row_b   <= row_a;

      // Stage C: activation + clamp, then write.
      qb_we <= valid_b;
      if (valid_b) begin
        qb_waddr <= row_b;
        for (int i = 0; i < npu_pkg::ARRAY_SIZE; i++) begin
          automatic logic signed [63:0] act_out;
          automatic logic signed [ 7:0] clamped;
          unique case (act_func)
            npu_pkg::ACT_RELU:        act_out = (requant_r[i] < 0) ? '0 : requant_r[i];
            npu_pkg::ACT_LEAKY_RELU:  act_out = (requant_r[i] < 0) ?
                                                (requant_r[i] >>> npu_pkg::LEAKY_RELU_SHIFT) : requant_r[i];
            default:                  act_out = requant_r[i];
          endcase
          if (act_out > 127)       clamped = 8'sd127;
          else if (act_out < -128) clamped = -8'sd128;
          else                     clamped = act_out[7:0];
          qb_wdata[i] <= clamped;
        end
      end

      if (state == ACT_PIPELINE && act_count < num_rows)
        ob_raddr <= act_count;
