...
inline int64_t numTilesFor(int64_t dim) {
  return (dim + kTileWidth - 1) / kTileWidth;
}
...
// (to fuse ReLU)
inline std::optional<mlir::tosa::ClampOp>
matchReluClamp(mlir::tosa::RescaleOp rescale) {
  if (!rescale->hasOneUse())
    return std::nullopt;
  ...
  if (!minAttr || !maxAttr)
    return std::nullopt;
  if (minAttr.getValue().getSExtValue() != 0)
    return std::nullopt;
  if (maxAttr.getValue().getSExtValue() < 127)
    return std::nullopt;
  return clamp;
}

inline mlir::Value logicalRescaleOutput(mlir::tosa::RescaleOp rescale) {
  if (auto clamp = matchReluClamp(rescale))
    return clamp->getResult();
...
inline bool feedsRescale(mlir::tosa::MatMulOp matmul) {
  for (mlir::Operation *user : matmul->getUsers()) {
    if (mlir::isa<mlir::tosa::RescaleOp>(user))
      return true;
    if (mlir::isa<mlir::tosa::AddOp>(user))
      for (mlir::Operation *addUser : user->getUsers())
        if (mlir::isa<mlir::tosa::RescaleOp>(addUser))
          return true;
  }
  return false;
}
