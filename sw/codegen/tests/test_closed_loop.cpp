...
uint8_t expected(int r, int c) {
  ...
  for (int k = 0; k < sim::kArraySize; ++k)
    acc +=
        static_cast<int64_t>(actA(r, k)) * static_cast<int64_t>(weightA(k, c));
  for (int k = 0; k < sim::kArraySize; ++k)
    acc +=
        static_cast<int64_t>(actB(r, k)) * static_cast<int64_t>(weightB(k, c));
...
int8_t chainAct0(int r, int k) {
  return static_cast<int8_t>((r * 3 + k) % 9 - 4);
}
int8_t chainWeight1(int k, int c) { return static_cast<int8_t>((k - c) % 5); }
int32_t chainBias1(int c) { return c - 4; }
int8_t chainWeight2(int k, int c) {
  return static_cast<int8_t>((k + c) % 6 - 3);
}
...
tosa::ConstOp buildChainI8Tensor(OpBuilder &builder, Location loc, int rows,
  ...
  for (int r = 0; r < rows; ++r)
    for (int c = 0; c < cols; ++c)
      data.push_back(gen(r, c));
  return tosa::ConstOp::create(
      builder, loc, ty, DenseElementsAttr::get(ty, ArrayRef<int8_t>(data)));
}
...
tosa::ConstOp buildChainI32Bias(OpBuilder &builder, Location loc, int cols,
  ...
  for (int c = 0; c < cols; ++c)
    data.push_back(gen(c));
  return tosa::ConstOp::create(
      builder, loc, ty, DenseElementsAttr::get(ty, ArrayRef<int32_t>(data)));
}
...
tosa::RescaleOp buildChainLayer(OpBuilder &builder, Location loc,
  ...
  if (bias) {
    auto add = tosa::AddOp::create(builder, loc, matmulOutTy,
  ...
  auto outTy =
      RankedTensorType::get(activationShape, builder.getIntegerType(8));
...
tosa::RescaleOp buildChainLayerN(OpBuilder &builder, Location loc,
  ...
  auto outputZp = buildChainScalarI8(builder, loc, 0);
  auto outTy =
      RankedTensorType::get({1, rows, cols}, builder.getIntegerType(8));
  return tosa::RescaleOp::create(
      builder, loc, outTy, rescaleInput, multiplierConst.getResult(),
      shiftConst.getResult(), inputZp.getResult(), outputZp.getResult(),
      /*scale32=*/true, tosa::RoundingMode::SINGLE_ROUND,
...
TEST(ClosedLoop, ThreeLayerChainFromTosaIsBitExact) {
  ...
  auto act0Const =
      buildChainI8Tensor(builder, loc, kChainRows, kChainK, chainAct0);
  auto weight1Const =
      buildChainI8Tensor(builder, loc, kChainK, kChainK, chainWeight1);
  auto bias1Const = buildChainI32Bias(builder, loc, kChainK, chainBias1);
  auto weight2Const =
      buildChainI8Tensor(builder, loc, kChainK, kChainK, chainWeight2);
  auto weight3Const =
      buildChainI8Tensor(builder, loc, kChainK, kChainK, chainWeight3);
  auto bias3Const = buildChainI32Bias(builder, loc, kChainK, chainBias3);

  auto rescale1 = buildChainLayer(
      builder, loc, act0Const.getResult(), weight1Const, bias1Const,
      static_cast<int32_t>(kL1Mult), static_cast<int8_t>(kL1Shift));
  auto rescale2 =
      buildChainLayer(builder, loc, rescale1.getResult(), weight2Const,
  ...
  auto writeI8Tile = [&](uint32_t addr, int rows, int cols,
                         int8_t (*gen)(int, int)) {
    std::vector<uint8_t> bytes;
    bytes.reserve(static_cast<size_t>(rows) * cols);
    for (int r = 0; r < rows; ++r)
      for (int c = 0; c < cols; ++c)
        bytes.push_back(static_cast<uint8_t>(gen(r, c)));
  ...
  std::vector<uint8_t> l1 =
      simulator.read(stores[0].getDdr3Addr(), kChainRows * kChainK);
  for (int r = 0; r < kChainRows; ++r)
    for (int c = 0; c < kChainK; ++c)
      EXPECT_EQ(l1[r * kChainK + c], chainLayer1Out(r, c))
          << "L1 row " << r << " c " << c;

  std::vector<uint8_t> l2 =
      simulator.read(stores[1].getDdr3Addr(), kChainRows * kChainK);
  for (int r = 0; r < kChainRows; ++r)
    for (int c = 0; c < kChainK; ++c)
      EXPECT_EQ(l2[r * kChainK + c], chainLayer2Out(r, c))
          << "L2 row " << r << " c " << c;
...
int8_t chainNKAct0(int r, int k) {
  return static_cast<int8_t>((r * 7 + k) % 9 - 4);
}
int8_t chainNKWeight1(int k, int c) {
  return static_cast<int8_t>((k + 2 * c) % 7 - 3);
}
...
uint8_t chainNKRequantizePassthrough(int64_t acc, uint32_t m, uint32_t shift) {
  int64_t scaled = acc * static_cast<int64_t>(m);
  if (shift > 0)
    scaled += static_cast<int64_t>(1) << (shift - 1);
  int64_t requant = scaled >> shift;
  return static_cast<uint8_t>(std::clamp<int64_t>(requant, -128, 127));
}
...
TEST(ClosedLoop, NTiledChainIntoKTiledConsumerIsBitExact) {
  ...
  auto act0Const =
      buildChainI8Tensor(builder, loc, kChainNKRows, 14, chainNKAct0);
  auto weight1Const =
      buildChainI8Tensor(builder, loc, 14, kChainNKN1, chainNKWeight1);
  ...
  auto rescale1 = buildChainLayerN(
      builder, loc, act0Const.getResult(), weight1Const, bias1Const,
      static_cast<int32_t>(kNK1Mult), static_cast<int8_t>(kNK1Shift));
  buildChainLayerN(builder, loc, rescale1.getResult(), weight2Const, bias2Const,
                   static_cast<int32_t>(kNK2Mult),
                   static_cast<int8_t>(kNK2Shift));
  ...
  for (int n = 0; n < 2; ++n) {
    ...
    for (int c = 0; c < 14; ++c) {
      ...
      for (int b = 0; b < 4; ++b)
        bBytes.push_back(static_cast<uint8_t>((v >> (8 * b)) & 0xFF));
    }
  ...
  for (int k = 0; k < 2; ++k) {
    ...
    simulator.write(loadWeights[2 + k].getDdr3Addr(), wBytes.data(),
                    wBytes.size());
  }
  ...
  std::vector<uint8_t> l1n0 =
      simulator.read(stores[0].getDdr3Addr(), kChainNKRows * 14);
  std::vector<uint8_t> l1n1 =
      simulator.read(stores[1].getDdr3Addr(), kChainNKRows * 14);
  for (int r = 0; r < kChainNKRows; ++r) {
    for (int c = 0; c < 14; ++c) {
      EXPECT_EQ(l1n0[r * 14 + c], chainNKLayer1Out(r, c))
          << "L1 N-tile0 row " << r << " c " << c;
  ...
  std::vector<uint8_t> result =
      simulator.read(stores[2].getDdr3Addr(), kChainNKRows * kChainNKN2);
...
int8_t kTileAct(int r, int k) {
  return static_cast<int8_t>((r * 5 + k) % 11 - 5);
}
...
TEST(ClosedLoop, KTiledMatmulFromTosaIsBitExact) {
  ...
  for (int r = 0; r < kKTileRows; ++r)
    for (int64_t k = 0; k < kKTileK; ++k)
      aData.push_back(kTileAct(r, static_cast<int>(k)));
  auto a = tosa::ConstOp::create(
      builder, loc, aTy, DenseElementsAttr::get(aTy, ArrayRef<int8_t>(aData)));

  std::vector<int8_t> bData;
  for (int64_t k = 0; k < kKTileK; ++k)
    for (int c = 0; c < 14; ++c)
      bData.push_back(kTileWeight(static_cast<int>(k), c));
  ...
  for (int c = 0; c < 14; ++c)
    biasData.push_back(kTileBias(c));
  ...
  auto shiftConst = tosa::ConstOp::create(
      builder, loc, zpTy,
      DenseElementsAttr::get(zpTy, static_cast<int8_t>(kKTileShift)));
  ...
  for (int tile = 0; tile < 2; ++tile) {
    ...
    for (int k = 0; k < 14; ++k)
      for (int c = 0; c < 14; ++c)
        wBytes.push_back(static_cast<uint8_t>(kTileWeight(tile * 14 + k, c)));
    simulator.write(loadWeights[tile].getDdr3Addr(), wBytes.data(),
                    wBytes.size());

    std::vector<uint8_t> aBytes;
    for (int r = 0; r < kKTileRows; ++r)
      for (int k = 0; k < 14; ++k)
        aBytes.push_back(static_cast<uint8_t>(kTileAct(r, tile * 14 + k)));
    simulator.write(loadInputs[tile].getDdr3Addr(), aBytes.data(),
                    aBytes.size());
  }
  ...
  std::vector<uint8_t> result =
      simulator.read(stores[0].getDdr3Addr(), kKTileRows * 14);
  ASSERT_EQ(result.size(), static_cast<size_t>(kKTileRows * 14));
  for (int r = 0; r < kKTileRows; ++r) {
    for (int c = 0; c < 14; ++c) {
      EXPECT_EQ(result[r * 14 + c], kTileExpected(r, c))
          << "row " << r << " channel " << c;
    }
  }
}
...
TEST(ClosedLoop, PartialKTileZeroPadsCorrectly) {
  ...
  for (int r = 0; r < kPartialRows; ++r)
    for (int64_t k = 0; k < kPartialK; ++k)
      aData.push_back(partialAct(r, static_cast<int>(k)));
  auto a = tosa::ConstOp::create(
      builder, loc, aTy, DenseElementsAttr::get(aTy, ArrayRef<int8_t>(aData)));
...
int8_t partialNAct(int r, int k) {
  return static_cast<int8_t>((r * 7 + k) % 9 - 4);
}
int8_t partialNWeight(int k, int c) {
  return static_cast<int8_t>((k + 2 * c) % 5 - 2);
}
...
uint8_t partialNExpected(int r, int c) { // c ranges over the true N=20
  ...
  for (int k = 0; k < 14; ++k)
    acc += static_cast<int64_t>(partialNAct(r, k)) *
           static_cast<int64_t>(partialNWeight(k, c));
...
TEST(ClosedLoop, PartialNTileZeroPadsCorrectly) {
  ...
  for (int r = 0; r < kPartialNRows; ++r)
    for (int k = 0; k < 14; ++k)
      aData.push_back(partialNAct(r, k));
  auto a = tosa::ConstOp::create(
      builder, loc, aTy, DenseElementsAttr::get(aTy, ArrayRef<int8_t>(aData)));

  std::vector<int8_t> bData;
  for (int k = 0; k < 14; ++k)
    for (int64_t c = 0; c < kPartialN; ++c)
      bData.push_back(partialNWeight(k, static_cast<int>(c)));
  auto b = tosa::ConstOp::create(
      builder, loc, bTy, DenseElementsAttr::get(bTy, ArrayRef<int8_t>(bData)));
  ...
  auto bZp = tosa::ConstOp::create(
      builder, loc, zpTy, DenseElementsAttr::get(zpTy, static_cast<int8_t>(0)));
  auto matmulOutTy =
      RankedTensorType::get({1, kPartialNRows, kPartialN}, i32Ty);
  auto matmul =
      tosa::MatMulOp::create(builder, loc, matmulOutTy, a.getResult(),
                             b.getResult(), aZp.getResult(), bZp.getResult());
  ...
  std::vector<int32_t> biasData;
  for (int64_t c = 0; c < kPartialN; ++c)
    biasData.push_back(partialNBias(static_cast<int>(c)));
  auto bias = tosa::ConstOp::create(
      builder, loc, biasTy,
      DenseElementsAttr::get(biasTy, ArrayRef<int32_t>(biasData)));
  auto add = tosa::AddOp::create(builder, loc, matmulOutTy, matmul.getResult(),
  ...
  auto shiftConst = tosa::ConstOp::create(
      builder, loc, zpTy,
      DenseElementsAttr::get(zpTy, static_cast<int8_t>(kPartialNShift)));
  auto rescaleOutTy =
      RankedTensorType::get({1, kPartialNRows, kPartialN}, i8Ty);
  tosa::RescaleOp::create(builder, loc, rescaleOutTy, add.getResult(),
                          multiplierConst.getResult(), shiftConst.getResult(),
                          aZp.getResult(), aZp.getResult(),
                          /*scale32=*/true, tosa::RoundingMode::SINGLE_ROUND,
  ...
  for (int r = 0; r < kPartialNRows; ++r)
    for (int c = 0; c < 14; ++c)
      EXPECT_EQ(result0[r * 14 + c], partialNExpected(r, c))
          << "row " << r << " channel " << c;
...
int8_t nTileAct(int r, int k) {
  return static_cast<int8_t>((r * 5 + k) % 9 - 4);
}
int8_t nTileWeight(int k, int c) {
  return static_cast<int8_t>((k + c) % 7 - 3);
}
...
uint8_t nTileExpected(int r, int c) { // c ranges over the true N=28
  ...
  for (int k = 0; k < 14; ++k)
    acc += static_cast<int64_t>(nTileAct(r, k)) *
           static_cast<int64_t>(nTileWeight(k, c));
...
TEST(ClosedLoop, NTiledMatmulFromTosaIsBitExact) {
  ...
  for (int r = 0; r < kNTileRows; ++r)
    for (int k = 0; k < 14; ++k)
      aData.push_back(nTileAct(r, k));
  auto a = tosa::ConstOp::create(
      builder, loc, aTy, DenseElementsAttr::get(aTy, ArrayRef<int8_t>(aData)));

  std::vector<int8_t> bData;
  for (int k = 0; k < 14; ++k)
    for (int64_t c = 0; c < kNTileN; ++c)
      bData.push_back(nTileWeight(k, static_cast<int>(c)));
  auto b = tosa::ConstOp::create(
      builder, loc, bTy, DenseElementsAttr::get(bTy, ArrayRef<int8_t>(bData)));
  ...
  std::vector<int32_t> biasData;
  for (int64_t c = 0; c < kNTileN; ++c)
    biasData.push_back(nTileBias(static_cast<int>(c)));
  auto bias = tosa::ConstOp::create(
      builder, loc, biasTy,
      DenseElementsAttr::get(biasTy, ArrayRef<int32_t>(biasData)));
  auto add = tosa::AddOp::create(builder, loc, matmulOutTy, matmul.getResult(),
  ...
  auto shiftConst = tosa::ConstOp::create(
      builder, loc, zpTy,
      DenseElementsAttr::get(zpTy, static_cast<int8_t>(kNTileShift)));
  ...
  for (int r = 0; r < kNTileRows; ++r)
    for (int k = 0; k < 14; ++k)
      actBytes.push_back(static_cast<uint8_t>(nTileAct(r, k)));
  simulator.write(loadInputs[0].getDdr3Addr(), actBytes.data(),
                  actBytes.size());
  simulator.write(loadInputs[1].getDdr3Addr(), actBytes.data(),
                  actBytes.size());
  ...
  for (int tile = 0; tile < 2; ++tile) {
    ...
    for (int k = 0; k < 14; ++k)
      for (int localC = 0; localC < 14; ++localC)
        wBytes.push_back(
            static_cast<uint8_t>(nTileWeight(k, tile * 14 + localC)));
    simulator.write(loadWeights[tile].getDdr3Addr(), wBytes.data(),
                    wBytes.size());
    ...
    simulator.write(loadBiases[tile].getDdr3Addr(), biasBytes.data(),
                    biasBytes.size());
  }
  ...
  for (int tile = 0; tile < 2; ++tile) {
    std::vector<uint8_t> result =
        simulator.read(stores[tile].getDdr3Addr(), kNTileRows * 14);
...
int8_t holdAct(int r, int k) {
  return static_cast<int8_t>((r * 7 + k) % 13 - 6);
}
...
uint8_t holdExpected(int r, int c) {
  ...
  for (int k = 0; k < 14; ++k)
    acc += static_cast<int64_t>(holdAct(r, k)) *
           static_cast<int64_t>(holdWeight(k, c));
...
TEST(ClosedLoop, MChunkedWeightHoldMatmulFromTosaIsBitExact) {
  ...
  for (int64_t r = 0; r < kHoldRows; ++r)
    for (int k = 0; k < 14; ++k)
      aData.push_back(holdAct(static_cast<int>(r), k));
  auto a = tosa::ConstOp::create(
      builder, loc, aTy, DenseElementsAttr::get(aTy, ArrayRef<int8_t>(aData)));
  ...
  for (int k = 0; k < 14; ++k)
    for (int c = 0; c < 14; ++c)
      bData.push_back(holdWeight(k, c));
  auto b = tosa::ConstOp::create(
      builder, loc, bTy, DenseElementsAttr::get(bTy, ArrayRef<int8_t>(bData)));
  ...
  auto shiftConst = tosa::ConstOp::create(
      builder, loc, zpTy,
      DenseElementsAttr::get(zpTy, static_cast<int8_t>(kHoldShift)));
  ...
  for (int k = 0; k < 14; ++k)
    for (int c = 0; c < 14; ++c)
      wBytes.push_back(static_cast<uint8_t>(holdWeight(k, c)));
  ...
  for (int chunk = 0; chunk < 2; ++chunk) {
    ...
    for (int64_t r = 0; r < chunkRowCounts[chunk]; ++r)
      for (int k = 0; k < 14; ++k)
        aBytes.push_back(
            static_cast<uint8_t>(holdAct(static_cast<int>(rowOffset + r), k)));
    simulator.write(loadInputs[chunk].getDdr3Addr(), aBytes.data(),
                    aBytes.size());
  ...
  for (int chunk = 0; chunk < 2; ++chunk) {
    ...
    for (int64_t r = 0; r < chunkRowCounts[chunk]; ++r) {
      for (int c = 0; c < 14; ++c) {
        EXPECT_EQ(result[r * 14 + c],
                  holdExpected(static_cast<int>(rowOffset + r), c))
...
TEST(ClosedLoop, WeightHoldWithKTilingFromTosaIsBitExact) {
  ...
  for (int64_t k = 0; k < kHKK; ++k)
    for (int c = 0; c < 14; ++c)
      bData.push_back(hkWeight(static_cast<int>(k), c));
  auto b = tosa::ConstOp::create(
      builder, loc, bTy, DenseElementsAttr::get(bTy, ArrayRef<int8_t>(bData)));
  ...
  auto shiftConst = tosa::ConstOp::create(
      builder, loc, zpTy,
      DenseElementsAttr::get(zpTy, static_cast<int8_t>(kHKShift)));
  ...
  for (int k = 0; k < 2; ++k) {
    ...
    for (int c = 0; c < 3; ++c) {
      ...
      simulator.write(loadInputs[loadInputIdx].getDdr3Addr(), aBytes.data(),
                      aBytes.size());
  ...
  for (int c = 0; c < 3; ++c) {
    std::vector<uint8_t> result =
        simulator.read(stores[c].getDdr3Addr(), chunkRows[c] * 14);
    ASSERT_EQ(result.size(), static_cast<size_t>(chunkRows[c] * 14));
    for (int64_t r = 0; r < chunkRows[c]; ++r) {
      for (int cc = 0; cc < 14; ++cc) {
        EXPECT_EQ(result[r * 14 + cc],
                  hkExpected(static_cast<int>(rowOffset + r), cc))
...
int8_t hc2Act0(int r, int k) {
  return static_cast<int8_t>((r * 3 + k) % 11 - 5);
}
...
TEST(ClosedLoop, ChainedHeldBatchProducerAndConsumerIsBitExact) {
  ...
  auto weight1Const =
      buildChainI8Tensor(builder, loc, kHC2K1, kHC2N1, hc2Weight1);
  auto bias1Const = buildChainI32Bias(builder, loc, kHC2N1, hc2Bias1);
  auto weight2Const =
      buildChainI8Tensor(builder, loc, kHC2N1, kHC2N2, hc2Weight2);
  auto bias2Const = buildChainI32Bias(builder, loc, kHC2N2, hc2Bias2);

  auto rescale1 = buildChainLayerN(
      builder, loc, act0Const.getResult(), weight1Const, bias1Const,
      static_cast<int32_t>(kHC2Mult1), static_cast<int8_t>(kHC2Shift1));
  buildChainLayerN(builder, loc, rescale1.getResult(), weight2Const, bias2Const,
                   static_cast<int32_t>(kHC2Mult2),
                   static_cast<int8_t>(kHC2Shift2));
  ...
  for (int k = 0; k < 2; ++k) {
    ...
    simulator.write(loadWeights[4 + k].getDdr3Addr(), wBytes.data(),
                    wBytes.size());
  }
  ...
  simulator.write(loadBiases[2].getDdr3Addr(), bias2Bytes.data(),
                  bias2Bytes.size());
  ...
  // Layer 2's final 3 chunks.
  {
    ...
    for (int c = 0; c < 3; ++c) {
      ...
      for (int64_t r = 0; r < chunkRows[c]; ++r) {
        for (int cc = 0; cc < 14; ++cc) {
          EXPECT_EQ(result[r * 14 + cc],
                    hc2Layer2Out(static_cast<int>(rowOffset + r), cc))
