...
TEST(InstructionEncoding, RoundtripPreservesAllFields) {
  Instruction original{Opcode::Matmul, true, 0x3, 0x00ABCDE, 0x1234, 0x5, 0x67};
...
TEST(InstructionEncoding, OpcodeOccupiesTopFourBits) {
  Instruction instr{static_cast<Opcode>(0xF), false, 0, 0, 0, 0, 0};
...
TEST(InstructionEncoding, FieldsDoNotBleedIntoEachOther) {
  Instruction instr{Opcode::Sync, true, 0x7, 0xFFFFFFF, 0xFFFF, 0xF, 0xFF};
  uint64_t encoded = instr.encode();
  Instruction decoded = Instruction::decode(encoded);
  ...
  EXPECT_EQ(decoded.reserved, 0xFu);
  EXPECT_EQ(decoded.tile_params, 0xFFu);
}

TEST(InstructionEncoding, ZeroInstructionEncodesToZero) {
  Instruction instr{static_cast<Opcode>(0x0), false, 0, 0, 0, 0, 0};
...
TEST(InstructionEncoding, AllOpcodeValuesRoundtrip) {
  for (uint8_t op = 0; op <= 0x7; ++op) {
    Instruction instr{
        static_cast<Opcode>(op), false, 0, 0xABCDE, 0x1234, 0x5, 0x67};
